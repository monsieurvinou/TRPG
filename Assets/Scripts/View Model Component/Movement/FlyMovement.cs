...
public class FlyMovement : AbstractMovement
{
    public override IEnumerator Traverse(Tile tile)
    {
        ...
        float y = (this.GetTraversePathMaxHeight(tile) + 1) * Tile.stepHeight;
    ...
    /// <summary>
    /// Return the max height of a path
    /// </summary>
    /// <param name="tile">Destination tile</param>
    /// <returns></returns>
    protected float GetTraversePathMaxHeight(Tile tile)
    {
        float maxHeight = tile.height;
        Tile previousTile = tile.prev;

        while (previousTile)
        {
            if (previousTile.height > maxHeight)
                maxHeight = previousTile.height;
